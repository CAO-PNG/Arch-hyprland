...
![](./img/kernel-base-3.png)

64位下的内存分配

![](./img/kernel-base-4.png)
...
- `int commit_creds(struct cred *new)`：该函数用以将一个新的 cred 结构体应用到进程。
    ...
    当prepare_kernel_cred返回为空的时候，就会使得commit_creds创建一个null的结构体，而root组的id恰好是0,这样就达到
