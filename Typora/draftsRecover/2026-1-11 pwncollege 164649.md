...
如果需要使用它的vm环境，就需要在他的终端上使用`vm connect`,这样就会连接到远程的vm环境，更多选项可以使用`vm `
...
当然目前的是没有`kaslr`的，也就是`nokaslr`，而我们需要`cat /proc/kallsyms`就可以看到地址了，不过我们需要root权限，如果你使用的是pwncollege的终端里面的vm,那么你需要在右下角将这个点击一下，使其处于开锁的图标状态
