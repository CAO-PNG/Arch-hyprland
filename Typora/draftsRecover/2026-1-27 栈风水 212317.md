...
而这个函数是`libc`的函数，所以，它的返回地址也是`libc`上的地址，而它会覆盖在`rbp-0x10`的位置，也就是返回地址。那么，是不是只需要我们去调用start就会在栈上留下`libc`的地址，接下来我们只需要同时得到两个`libc`的地址，将地址低的那个修改成`libc` 中的`pop rdi ;ret`，然后紧挨着放一个got表的地址，另一个修改成`libc`里面的`puts`，中间使用ret连接起来，那么就可以泄漏地址，
